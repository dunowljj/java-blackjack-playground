...
* 결과 도출
  ...
  * 딜러 수익
    * -(플레이어의 수익들의 합)
### 오류
1. 딜러 초기화시 State가 Hit이기 때문에, 카드를 추가하지 않는 경우 Hit으로 끝난다.
2. 실행 오류 : 모두가 블랙잭이 아니고 버스트도 없을때, 딜러가 승자인데 플레이어 중 최고값이 승자가 됨.(Stay 고쳐도 난다)
3. nosuchelement가 뜸. Stay있는데도 뜬다.
pobi,jason,toby,baekjooon
4. participantsTest 짜고 실행 